...
app.get("/addTable", function(req, res) {
  /*Determine if client side data is empty and populate table with everything*/
        ...
        var data = [];
          ...
          /*Create table of data and sent to client*/
          var TransTable = createTable(Trans,true);
          res.write(""+TransTable); 
          res.end();
...
app.get("/addSQL", function(req, res) {
  /*Determine if client side data is empty and populate table with everything*/
        var table = req.query;
        var sql = "INSERT INTO STUDENT (studentId, firstname, lastname,birthdate,major) VALUES"+
        "\("+"\""+con.escape(table.studentID)+"\""+","+"\""+con.escape(table.FirstName)+"\""+","+"\""+ con.escape(table.LastName)+"\""+","+"\""+con.escape(table.BirthDate)+"\""+","+"\""+con.escape(table.Major)+"\""+"\)";
...
function createTable(TableData, edit = false){
		  ...
		  if(edit){
			...
			for(i = 1; i < TableData.length; i++){
				...
				for(j = 0; j < TableData[i].length; j++){
					dataTable+= "<td"+editable+">" + TableData[i][j] + "</td><br>";
				}
				dataTable += "</tr><br>";
